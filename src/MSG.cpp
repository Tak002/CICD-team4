...
bool sendMessage(const std::tuple<std::string, int, int, std::string>& msgData)
{
    ...
    const auto& [src_id, itemID, itemNum, newCertCode] = msgData;
    ...
    json ipjson;
    file >> ipjson;
    string ipaddress = ipjson[src_id];

    string msg_type = "req_prepay";

    string jsonstr = msgFormat(msg_type,src_id, std::to_string(itemID), std::to_string(itemNum), "","",newCertCode,"");
    std::cout << "[Send Message] " << msg_type << std::endl;
    json msg = json(jsonstr);
    clientMessage(src_id, msg);
