...
class MSG {
    ...
    public :
        ...
        std::tuple<int,int,std::string> DVMMessageOutofStock(); // 재고 부족시 메시지 전송 -> 1. broadcast로 클라이언트가 스레드를 만들어서 총 7개의 json 메시지를 받아온 후 2. 7개의 메시지를 바탕으로 Calc에서 거리를 계산 후 3. Nearest Position을 호출하면 된다.
